...
**⚠️ 关键方向（f2-05 实测 hint）**：若 hint 提示“字节码本身就是 flag 的构造过程”，
说明 **flag 不在算法里，就在字节码/数据段里**。此时优先：
1. `strings -a -n 4 binary` 直接搜 flag 片段/可疑字符串；
2. dump 字节码数组后**逐字节静态分析**（找常量、XOR 密钥、逐字符拼接/比较逻辑）；
3. **不要急着写完整 VM 模拟器**（那是 flag 藏在复杂校验里时才需要）。
先花 3 轮静态找，找不到再走 D.2 的三步法。

**f2-05 实测解剖（validator ELF 14.5KB，门禁+VM）**：
- access_code 校验在 main：`movabsq $0x6d6a031f1105170c` 小端字节 [0c 17 05 11 1f 03 6a 6d]，每字节 XOR 0x5a → **`VM_KEY07`**（8 字符，直接过门禁）
- VM 循环结构：r12=0x4010(字节码) r13=0x2004(跳转表) r15=0x401a(key) rcx=0x4020(数据)，PC 0..9，r14 计数器 0..30（31 轮）
- 跳转表索引 = `(opcode+0x3f)&0xff`（0xc1..0xcb），**0xcb=handlerH(条件跳回)、0xca=退出**（易搞反）；0xc3/0xc7/0xc8=skip2
- 每轮：读 0x4020[r14] → 读 0x401a[r14&3] → putc('.')（**被混淆成点，flag 字符计算被删成死代码**）→ r14++ → 跳回
- .data 文件偏移 = vaddr 0x4000 → off 0x3000（LOAD offset=0x2d90 vaddr=0x3d90）；字节码=c2 c1 c0 c7 d2 c6 c5 cb c3 ca，key(0x401a)=8d de b9 0f，data(0x4020)=37 34 2b cd e9 45 ...（前3字节恰是 ASCII '7''4''+'）
- **卡点**：flag[i]=0x4020[i] op 0x401a[i&3]，op 为 xor/add/sub 均不匹配 “flag{”（混淆器删除了组合运算，只留读操作死代码）→ 需 angr 符号执行或还原原始 C，不要死磕简单 op
- **patch 调试法（qemu 下 ptrace 不可用，无需 gdb）**：把 handler F 的 `movl $0x2e,%edi`（0x1373 的 `bf 2e 00 00 00`）patch 成 `42 0f b6 3c 31`（movzbl (%rcx,%r14),%edi = 读 0x4020[r14]），运行即见原始密文
- **暴力穷举结论**：c[i] op a[i&3]（xor/add/sub/and/or/位变换/rotl/rotr/swapnib/bitrev/&0x7f/^0x80/取反）+ 索引相关 + 三重（含 0x4040、字节码、0x2000）→ **均不匹配 “flag{”，最高可读性 19/31**
- **关键矛盾**：0x4020[0..2]='7','4','+'（ASCII高位0），但 0x401a[0..2]=0x8d,0xde,0xb9（高位1）→ 若 XOR 前3字节必变非 ASCII → **0x401a 不是直接 XOR key**；flag 公式可能涉及死数据 0x4040（0xc0-0xcb 降序）或 0x2000（[01,00,02,00]）或链式状态

> **最短成功链（公式被删时）**  
> 1. 过门禁（如上 XOR → `VM_KEY07`）  
> 2. 确认 putc 只吐 `.` / 组合运算成死代码 → **停止**枚举 xor/add/sub  
> 3. 在 **解题容器**按需 `pip install angr`（默认镜像为控体积未预装；已有 z3）  
> 4. angr：对输入符号化，`find` 打印含 `flag{` 的路径 / 约束前缀 `flag{`，一次解 31 字节  
> 5. 备选：对照未混淆的同类 C 模板重写输出函数再编——不要继续手推已删公式  
